    ...
    try {
        ...
        materials.forEach((material) => {
            ...
            if(remainingWeight >= material.weight) {
                // EL MATERIAL QUE ENTRA EN LA CARGA SE AGREGA A UN ARRAY DE OBJETOS DE MATERIALES
        ...
        if(response.length == 0) return {message: 'NO HAY RUTA A TOMAR PARA ESTA SITUACION', status: 409}
...
export const validateMaterials = async (materials: IMaterial[]) => {
    try {
        ...
        if(nonExistent.length > 0) return {message: `LOS SIGUIENTES MATERIALES NO EXISTEN: ${JSON.stringify(nonExistent)}`, status: 404}
    } catch (error) {
